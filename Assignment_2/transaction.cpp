...
bool txn_t::validate()
{
    ...
    SHA256_Update(&sha256_1, public_key.data(), public_key.size());
    SHA256_Final(h1, &sha256_1);

    SHA256_CTX sha256_2;
    SHA256_Init(&sha256_2);
    SHA256_Update(&sha256_2, public_key.data(), public_key.size());
    SHA256_Update(&sha256_2, source_addr.data(), source_addr.size());
    SHA256_Update(&sha256_2, dest_addr.data(), dest_addr.size());
    SHA256_Update(&sha256_2, change_addr.data(), change_addr.size());
    SHA256_Update(&sha256_2, &amount, sizeof(amount));
    SHA256_Final(h2, &sha256_2);

    bool val1 = true;
    for(unsigned i=0; i != SHA256_DIGEST_LENGTH; i++)
      val1 = val1 && (h1[i] == source_addr.data()[i]);

    bool val2 = true;
    for(unsigned i =0; i != SHA256_DIGEST_LENGTH; i++)
      val2 = val2 && (h2[i] == tx_hash.data()[i]);
